...
class Evaluator(MyRiceImgReader):
    def __init__(self,
        ...
        cmd                   = lambda spc,t='tranformers': ['unzip','-p',Zip,f'{parent}/{t}/{spc}.joblib']
        self.tranformers      = { key : cmd(spc)

                                for spc,key in self.all_species.items()
            }
        print(self.tranformers)
        self.tranformers      = { key : joblib.load(BytesIO(subprocess.run(cmd(spc),stdout=subprocess.PIPE).stdout))  

                                for spc,key in self.all_species.items()
            }
        self.inverse_tranformers  = { key : joblib.load(BytesIO(subprocess.run(cmd(spc,'inverse_transformers'),stdout=subprocess.PIPE).stdout))  
